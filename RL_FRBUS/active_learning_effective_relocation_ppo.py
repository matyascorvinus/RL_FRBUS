...
import numpy as np
from ppo_agent import FederalReservePPOAgent, PPOAgent
...
class ActiveLearningEffectiveRelocationPPOAgent(PPOAgent):
    ...
    def should_relocate(self, previous_quarter, current_quarter):
        ...
        if self.method == "Bored":
            self.reloc_prob = self.calculate_relocation_probability_bored(current_value, previous_value)
        else:  # "InTrouble"
            self.reloc_prob = self.calculate_relocation_probability_in_trouble(current_value, previous_value)
        
        # Decide whether to relocate
        if np.random.random() < self.reloc_prob:
...
class FederalReserveActiveLearningEffectiveRelocationPPOAgent(FederalReservePPOAgent):
    def __init__(self, state_dim, action_dim, current_quarter="1970q1", hidden_dim=256, uncertainty_model_dim=256, lr=3e-4, gamma=0.99, eps_clip=0.2, K_epochs=10, seed=42, phi=100):
        super(FederalReserveActiveLearningEffectiveRelocationPPOAgent, self).__init__(
            state_dim, action_dim, hidden_dim, lr, gamma, eps_clip, K_epochs, seed
        ) 
